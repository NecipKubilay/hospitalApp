...
namespace HospitalApp.Forms
{
    public partial class frmRandevuOlustur : Form
    {
        ...
        private void frmRandevuOlustur_Load(object sender, EventArgs e)
        {
            ...
            // Doktor isimlerini Doktor tablosundan al
            SqlCommand komut = new SqlCommand("select DoktorAd from Doktor", baglan.baglanti());
            SqlDataReader sqlDataReader = komut.ExecuteReader();
            while (sqlDataReader.Read())
            {
                cmbbxDoktor.Items.Add(sqlDataReader[0].ToString());
            }
            sqlDataReader.Close(); // Önemli: Okuyucuyu kapat

            // Branş isimlerini Brans tablosundan al
            SqlCommand komut1 = new SqlCommand("select BransAd from Brans", baglan.baglanti());
            sqlDataReader = komut1.ExecuteReader(); // Yeni bir okuyucu oluştur
            while (sqlDataReader.Read())
            {
